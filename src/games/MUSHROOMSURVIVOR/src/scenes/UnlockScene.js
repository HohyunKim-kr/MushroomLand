...
export default class UnlockScene extends Phaser.Scene {
    ...
    create() {
        ...
        // 배경: 캔버스 크기에 맞게 설정
        this.background = this.add.rectangle(width / 2, height / 2, width, height, 0x333333);
        ...
        this.titleText = this.add.text(width / 2, height * 0.2, this.win ? 'You Survived!' : 'Game Over', {
        ...
        this.statsText = this.add.text(width / 2, height * 0.35, `Coins: ${coins}\nEnemies Killed: ${kills}`, {
        ...
        if (kills >= 10 && !unlockedWeapons.includes('missile')) {
            ...
            this.unlockText = this.add.text(width / 2, height * 0.5, `Unlocked: Missile!`, {
                fontSize: `${unlockFontSize}px`,
                color: '#0f0',
            }).setOrigin(0.5);
        }

        this.restartButton = this.add.text(width / 2, height * 0.65, 'Back to Menu', {
